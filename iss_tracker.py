...
def read_iss_data() -> List[dict]:
    ...
    """
    # NASA ISS DATA url for XML format
    ...
    # terminate early for invalid paramter inputs
...
def compute_nearest_geolocation(lat: float, lon: float) -> str:
    ...
    geocoder = Nominatim(user_agent='iss_tracker', timeout = 5)
...
def epoch_location(epoch: str) -> dict:
    ...
    return {'latitude':lat, 'longitude': lon, 'altitude':alt, 'Nearest_Geolocation':geolocation}
